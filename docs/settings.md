...
| `OMP_SESSION_STORAGE`   | `session.storage`           | Env value takes precedence over config.                                                           |
| `OMP_SESSION_SQL_DSN_FILE` | `session.sql.dsnFile`    | Env value takes precedence over config.                                                           |
...
| `session.storage`                   | enum    | `file`    | `file`, `sql`. Where session transcripts persist: the JSONL tree under the sessions directory, or one row per session in a PostgreSQL/MySQL/SQLite table (`omp_session_files`, created on first use). Overridden by `OMP_SESSION_STORAGE`. Every session factory, `--resume`/`--continue`, the session picker, and listing use the selected store. See [Session storage](./session.md#storage-abstractions). |
| `session.sql.dsnFile`               | string  | _(unset)_ | Path of a file whose trimmed contents are the database connection URL (`postgres://…`, `mysql://…`, `sqlite:…`); the dialect follows the URL scheme. Required when `session.storage` is `sql`: a missing, unreadable, or blank file, or an unreachable database, refuses to start naming the variable or setting and the path — never the connection string, and never a silent fallback to files. Overridden by `OMP_SESSION_SQL_DSN_FILE`. |
