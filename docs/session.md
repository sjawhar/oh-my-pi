...
The CLI resolves one process-wide default storage at start-up (`resolveSessionStorage` in `session-storage-config.ts`, installed with `setDefaultSessionStorage`) before any session is opened; every `SessionManager` factory, `resolveResumableSession`, the session picker, and recent-session listing fall back to `defaultSessionStorage()` when no storage is passed, so a session written to SQL is found by `--resume`, `--continue`, and the picker without a second path. `omp gc` is on-disk maintenance of the sessions tree, archive, and blob directories; it does not read `session.storage`.
...
- Tool artifacts and image blobs stay under the agent directory on local disk in either mode; only the transcript moves. `omp gc` (archive, blob, and WAL sweeps) keeps working on that on-disk tree and never touches the SQL table.
