...
describe("ExtensionAPI.askEphemeral", () => {
	...
	it("answers alongside a streaming primary turn without changing that turn's history or queues", async () => {
		...
		// Snapshot history only once the primary user message is committed: it lands in
		// session.messages on its message_end, which can follow the provider request starting.
		const primaryMessageCommitted = Promise.withResolvers<void>();
		const unsubscribe = session.subscribe(event => {
			if (event.type === "message_end" && event.message.role === "user") primaryMessageCommitted.resolve();
		});
		const primary = session.prompt("Continue the primary task");
		await primaryStarted.promise;
		await primaryMessageCommitted.promise;
	...
	it("rejects unavailable ephemeral calls without queuing a primary delivery", async () => {
		...
		// v18.3.2's runEphemeralTurn checks the signal before any provider request.
		await expect(aborted.api.askEphemeral({ prompt: "status?", signal: abortController.signal })).rejects.toThrow(
			"The operation was aborted.",
		);
		expect(aborted.mock.calls).toHaveLength(0);
